...
import { getRegistry, activateCards, getCardName, getActiveCardIds, saveRegistry } from "./card-manager";
...
export function createRPWebServer(
  ...
  function serveFile(urlPath: string, res: any, rpToken?: string) {
    ...
    if (ext === ".html" && rpToken) {
      ...
      content = content.toString().replace('</head>', tokenScript + '</head>');
    }
  ...
  function sanitizeEventForBroadcast(event: any): any {
    ...
    if (!msg || msg.role !== 'assistant') return event;
    ...
    try {
      switch (command.type) {
        ...
        case "get_append_system": {
          // 读取 APPEND_SYSTEM.md 返回给前端
          const appendPath = join(stateDir, "APPEND_SYSTEM.md");
          let content = "";
          if (existsSync(appendPath)) {
            content = readFileSync(appendPath, "utf-8");
          }
  ...
  async function start(ctx: any) {
    ...
    rpWss.on("connection", (ws: any) => {
      ...
      // 下发卡片 UI 组件（供前端动态加载）
      try {
        const { getActiveCards } = require("./card-manager");
