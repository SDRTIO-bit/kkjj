...
test("CORE_CHARS 包含 6 个角色", () => {
  assert.equal(CORE_CHARS.length, 6);
});

test("CORE_CHARS 无重复", () => {
  const unique = new Set(CORE_CHARS);
  assert.equal(unique.size, CORE_CHARS.length);
});
...
function processPeriodicEventsImpl(state, daysPassed) {
  const events = [];
...
test("花开蒂落：归属值≥60且未触发时触发", () => {
  ...
  const events = processPeriodicEventsImpl(state, 1);
