...
function generateWorldbookEntries(characterData, charName, worldbookDir) {
  ...
  if (!book || !book.entries || book.entries.length === 0) {
    console.log(`   ℹ️ 角色 "${charName}" 无 character_book，跳过世界书条目生成`);
    return 0;
  }

  // 创建角色专属世界书目录
  const charBookDir = join(worldbookDir, '角色设定');
  ensureDir(charBookDir);

  // 全部导入：启用条目放在根目录，禁用条目放在 disabled/ 子目录
  // 禁用的通常是不同路线的世界书，保留供后续路线切换
  const enabledEntries = book.entries.filter(e => e.enabled !== false);
  const disabledEntries = book.entries.filter(e => e.enabled === false);

  let count = 0;
  let disabledCount = 0;

  // 处理所有条目（启用 + 禁用），禁用的标记到 disabled 子目录
  for (const group of [
    { entries: enabledEntries, subDir: '', label: '启用' },
    { entries: disabledEntries, subDir: 'disabled', label: '禁用（不同路线备用）' },
  ]) {
    if (group.entries.length === 0) continue;

    const targetDir = group.subDir ? join(charBookDir, group.subDir) : charBookDir;
    ensureDir(targetDir);
    ...
    for (const entry of group.entries) {
      ...
      const priority = entry.priority ?? entry.insertion_order ?? count;
      ...
      const frontMatter = [
        ...
        `selective: ${selective}`,
        `enabled: ${enabled}`,
        secondaryKeys.length > 0 ? `secondary_keys: [${secondaryKeys.map(k => `"${k}"`).join(', ')}]` : '',
      ...
      while (existsSync(finalPath)) {
        finalPath = join(targetDir, `${safeName}_${suffix}.md`);
        suffix++;
      }

      writeFileSync(finalPath, `${frontMatter}\n\n# ${comment}\n\n${content}\n`, 'utf-8');
      count++;
    }

    if (group.subDir) {
      disabledCount = group.entries.length;
    }
  }

  const total = enabledEntries.length + disabledEntries.length;
  if (disabledCount > 0) {
    console.log(`   ✅ 生成 ${enabledEntries.length} 个启用条目 + ${disabledCount} 个禁用条目（共 ${total} 条） → ${charBookDir}`);
  } else {
    console.log(`   ✅ 生成 ${count} 个世界书条目 → ${charBookDir}`);
  }
  return count;
}
...
function importCharacterCard(filePath, targetDir) {
  ...
  console.log(`   - 角色描述: ${cardWorldbook}/角色设定/${charName}_角色描述.md`);
  if (entryCount > 0) {
    console.log(`   - 世界书条目: ${cardWorldbook}/角色设定/ （${entryCount} 个文件）`);
  }
